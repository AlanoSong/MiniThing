...
void
MiniThingCore::SetDataBasePath(std::wstring dbName)
{
...
void
MiniThingCore::InitLogger(std::wstring& logPath)
{
    /* close printf buffer */
    setbuf(stdout, NULL);
...
HRESULT
MiniThingCore::StartInstance(PFN_UPDATE_STATUS_CB cb)
{
    HRESULT ret = S_OK;

    // 1. Setup chinese print env
    SetChsPrintEnv();

    // 2. Get current app path
    GetCurrentPath(m_appCurrentPath);
...
HRESULT
MiniThingCore::QueryAllVolume(void)
{
    HRESULT ret = S_OK;

    wchar_t logicDrvStrs[1024];
    wchar_t* drvStr;
    RtlZeroMemory(logicDrvStrs, 1024);

    GetLogicalDriveStrings(1024 - 1, logicDrvStrs);
    drvStr = (wchar_t*)logicDrvStrs;

    while (*drvStr)
    {
        VolumeInfo volInfo;
        std::wstring name = drvStr;
        volInfo.volumeName = name.substr(0, name.length() - 1);

        if (this->IsNtfs(volInfo.volumeName))
        {
            log_i("Add vol: %s", WstringToString(volInfo.volumeName).c_str());
            m_volumeSet.push_back(volInfo);
        }

        drvStr += (_tcslen(drvStr) + 1);
    }
...
HRESULT
MiniThingCore::GetAllVolumeHandle()
{
    HRESULT ret = S_OK;
    ...
    for (auto it = m_volumeSet.begin(); it != m_volumeSet.end();)
    {
        ...
        {
            log_w("Get handle failed: %s", WstringToString(it->volumeName).c_str());

            // Remove this one cause iterator been invalid
            it = m_volumeSet.erase(it);
        }
        else
        {
            log_w("Get handle success: %s", WstringToString(it->volumeName).c_str());
            it++;
        }
    }
...
void
MiniThingCore::CloseVolumeHandle(void)
{
...
bool
MiniThingCore::IsNtfs(std::wstring volName)
{
    bool isNtfs = false;
...
HRESULT
MiniThingCore::SortVolumeSetAndUpdateSql(void)
{
    HRESULT ret = S_OK;
...
HRESULT
MiniThingCore::SortVolumeAndUpdateSql(VolumeInfo& volume)
{
    HRESULT ret = S_OK;
...
MiniThingCore::DeleteUsn(void)
{
    ...
    for (auto it = m_volumeSet.begin(); it != m_volumeSet.end(); it++)
    {
        ...
        if (false == status)
        {
            log_w("Delete usn file failed : %s",
                WstringToString(it->volumeName).c_str());
            ret = E_FAIL;
        }
    }
...
HRESULT
MiniThingCore::CreateMonitorThread(void)
{
    HRESULT ret = S_OK;
...
void
MiniThingCore::StartMonitorThread(void)
{
}

void
MiniThingCore::StopMonitorThread(void)
{
    for (auto it = m_volumeSet.begin(); it != m_volumeSet.end(); it++)
    {
...
HRESULT
MiniThingCore::CreateQueryThread(void)
{
    HRESULT ret = S_OK;
...
void
MiniThingCore::StartQueryThread(void)
{
    ResumeThread(m_hQueryThread);
}

void
MiniThingCore::StopQueryThread(void)
{
...
MiniThingCore::SQLiteOpen(void)
{
    ...
    if (result == SQLITE_OK)
    {
        ...
        if (res == SQLITE_OK)
        {
            log_i("Data base create");
            m_isSqlExist = false;
        }
        else
        {
            log_i("Data base exist");
            m_isSqlExist = true;
        }
    }
    else
...
HRESULT
MiniThingCore::SQLiteInsert(UsnInfo* pUsnInfo)
{
    HRESULT ret = S_OK;
    ...
    sprintf_s(sql,
        "INSERT INTO UsnInfo VALUES(%llu, %llu, %lld, '%s', '%s');",
        pUsnInfo->pSelfRef,
        pUsnInfo->pParentRef,
        pUsnInfo->timeStamp.QuadPart,
        nameUtf8.c_str(),
        pathUtf8.c_str());
...
HRESULT
MiniThingCore::SQLiteQuery(std::wstring queryInfo, std::vector<std::wstring>& vec)
{
    HRESULT ret = S_OK;
    char sql[1024] = { 0 };
    char* errMsg = nullptr;
...
HRESULT
MiniThingCore::SQLiteQueryV2(QueryInfo* queryInfo, std::vector<UsnInfo>& vec)
{
    HRESULT ret = S_OK;
    char sql[1024] = { 0 };
    char* errMsg = nullptr;
...
HRESULT
MiniThingCore::SQLiteDelete(UsnInfo* pUsnInfo)
{
    HRESULT ret = S_OK;
...
HRESULT
MiniThingCore::SQLiteUpdate(UsnInfo* pOriInfo, UsnInfo* pNewInfo)
{
    HRESULT ret = S_OK;
...
HRESULT
MiniThingCore::SQLiteClose(void)
{
